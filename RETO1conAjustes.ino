...
const char ssid[] = "FamiliaAvilaGarcia";
//Contraseña de la red Wifi
const char pass[] = "120319Joseph";

//Usuario uniandes sin @uniandes.edu.co
#define HOSTNAME "a.avilar2"

//Conexión a Mosquitto
const char MQTT_HOST[] = "iotlab.virtual.uniandes.edu.co";
const int MQTT_PORT = 8082;
//Usuario uniandes sin @uniandes.edu.co
const char MQTT_USER[] = "a.avilar2";
//Contraseña de MQTT que recibió por correo
const char MQTT_PASS[] = "202116037";
...
void loop()
{
  ...
  const int B = 2;        //Resistencia a la luz (10 Lux) en KΩ
  const int Rc = 10;       //Resistencia calibracion en KΩ
  const int UNPUTRes = A0;   //Pin del LDR
  int V = analogRead(UNPUTRes);
  int l = ((long)V*A*10)/((long)B*Rc*(1024-V));
  ...
  if ( !isnan(h) && !isnan(t) ) {
    ...
    //Publica en el tópico de la iluminancia
